...
const Organizations = () => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      // Check editMode
      const url = editMode
        ? `organization/update-organization/${payload?.id || current?.id}`
              ...
              if (res.data.status) {
                ...
                notification.error({
                  ...
                  description:
                    ...
                    "Error creating Department, please try again or contact support.",
                });
              }
            },
            (error) => {
            ...
            (res) => {
              ...
              if (res.data.status) {
                ...
                notification.error({
                  ...
                  description:
                    res?.response?.data?.message ||
                    "Error Updating Department, please try again or contact support.",
                });
              }
            },
            (error) => {
  ...
  useEffect(() => {
    const list = organization?.map((el) => {
      return {
        key: el?.id,
        ...el
      };
  ...
  useEffect(() => {
    if (organization.length <= 0) fetchOrganizations();
  }, []);
    ...
    <>
      <AdminLayout title="Organizations">
        <div className="w-full relative px-6">
          <div className="py-6">
            <button
              className="h-10 text-white bg-primary rounded-lg border-0 py-2 px-4 focus:outline-none hover:bg-primary/90 text-lg space-x-4 inline-flex items-center"
      ...
      >
        <section className="w-full mx-auto bg-white">
          ...
          <Form
            form={form}
            className="py-6 bg-white rounded grid grid-cols-2 gap-5"
            onFinish={onFinish}
            autoComplete="off"
            layout="vertical"
          >
            <Form.Item
              className="mb-0"
              name="name"
              label={<span className="block text-base font-bold text-gray-700">Name</span>}
              rules={[
                { required: true, message: "Provide objective for this task" },
              ]}
            >
              <Input
                name="name"
            ...
            <Form.Item
              className="mb-0"
              name="address"
              label={<span className="block text-base font-bold text-gray-700">Address</span>}
              rules={[
                { required: true, message: "Provide address location" },
              ]}
            >
              <Input
                name="address"
            ...
            <Form.Item
              className="mb-0"
              name="contact_number"
              label={<span className="block text-base font-bold text-gray-700">Contact Number</span>}
              rules={[
                { required: true, message: "Enter name of department head " },
              ]}
            >
              <Input
                name="head"
                className="w-full px-3 py-3 text-base leading-tight text-gray-700 border border-primary/80 rounded appearance-none focus:outline-none focus:shadow-outline"
                type="number"
                placeholder="Provide organization contact number"
              />
            </Form.Item>

            <Form.Item
              className="mb-0"
              label={<span className="block text-base font-bold text-gray-700">Email</span>}
              name="email"
            ...
            >
              <Input
                ...
                type="email"
                className="w-full px-3 py-3 text-base leading-tight text-gray-700 border border-primary/80 rounded appearance-none focus:outline-none focus:shadow-outline"
                placeholder="Provide organization email"
              />
            </Form.Item>

            <Form.Item
              className="mb-0"
              label={<span className="block text-base font-bold text-gray-700">Country</span>}
              name="country"
            >
              <Select
                className="w-full text-base leading-tight"
                name="country"
                size="large"
                style={{ height: "44px" }}
                suffixIcon={
                  <svg width="12" height="9" viewBox="0 0 12 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M6.1548 8.71582L0.495195 0.0468597H11.8144L6.1548 8.71582Z" fill="#0883C7" />
                  </svg>
                }
              >
                {Countries?.map((el, index) => (
                  <Select.Option key={index} id={el.name} value={el.name}>
                    {el.name}
                  </Select.Option>
                ))}
              </Select>              
            ...
            <Form.Item
              className="mb-0"
              label={<span className="block text-base font-bold text-gray-700">Active</span>}
              name="is_active"
            >
              <Switch />
            </Form.Item>
